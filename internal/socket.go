...
import (
	...
	"github.com/EncrypteDL/Bybit-WebSocket-Connector/base/fill"
	"github.com/EncrypteDL/Bybit-WebSocket-Connector/base/market"
...
var (
	orderBookLocals = make(map[string]*market.OrderBookLocal)
)
		...
		for {
			...
			if strings.HasPrefix(channel, BybitWSOrderBookL2_25) {
				...
				case "snapshot":
					...
					if !ok {
						obl = market.NewOrderBookLocal()
						orderBookLocals[symbol] = obl
					}
					obl.LoadSnapshot(ob)
				case "delta":
					var obd *market.OrderBookL2Delta
				...
				res.Type = UNDEFINED
				res.Result = fmt.Errorf("%v", string(msg))
			}
